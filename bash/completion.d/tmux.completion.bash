...
function _tmux_complete_format() {
    local predefined="#S #I #P #W #T #I.#P #S:#I.#P #{pane_pid} #{pane_current_command} #{pane_cuurent_path}"
    local cur="${1}"
    COMPREPLY=( ${COMPREPLY[@]:-} $(compgen -W "$predefined" -- "${cur}") )
}
...
_tmux() {
    ...
    if [[ $COMP_CWORD -le $cmd_index ]]; then
        ...
        COMPREPLY=( ${COMPREPLY[@]:-} $(compgen -W "$(tmux start-server \; list-commands -F '#{command_list_name}')" -- "${cur}") )
    else
        cmd="$(tmux list-commands -F '#{command_list_alias}: #{command_list_name}' | command grep "^${cmd}:\| ${cmd}$" | awk '{print $2}')"
        case ${cmd} in
