...
reminder() { # @@ # {{{
  if [[ $1 == @@ ]]; then # {{{
    ...
    *) # {{{
      ...
      1) # {{{
        ...
        if [[ ! -z $REMINDER_PATH && -e $REMINDER_PATH ]]; then
          for i in $(cd "$REMINDER_PATH"; ls -d *); do
            [[ -f $REMINDER_PATH/$i && -x $REMINDER_PATH/$i ]] && ret+=" $i"
          done
        fi;; # }}}
      *) # {{{
        ret="30m 1h 16:30";; # }}}
  ...
  if [[ ! -z "$REMINDER_PATH" && -e "$REMINDER_PATH/$msg" ]]; then
    msg="$REMINDER_PATH/$msg"
  ...
  if ! $file && [[ $err == '0' ]]; then # {{{
    local msg_wrapper="$REMINDER_PATH/.msg.sh"
