...
tm() { # @@ # {{{
  ...
  session) # {{{
    ...
    case $1 in
    -s | --save) # {{{
      $ENV_SCRIPTS/file-tools/update-file.sh $dst --update-line "^$name " "$(tm --l-dump --file -)";; # }}}
    -d | --delete) # {{{
      $ENV_SCRIPTS/file-tools/update-file.sh $dst --remove-line "^$name ";; # }}}
    -e | --edit) # {{{
      vim $dst;; # }}}
    -r | --restore) # {{{
      shift
        ...
        if [[ $do_cd == 'true' || $do_cd == 'auto' ]]; then
          if [[ $i -lt ${#pNow[*]} || $do_cd == 'auto' ]]; then
            ...
            if ( $setCdAll ) || ( $setPane1 && [[ $paneNo == 1 ]] ) || ( [[ $panePids == 1 ]] ); then
              tmux send-keys -t "$dst.$paneNo" " cd \"${paths[i]}\"; env-snapshot sync; clear"
            else
  ...
  attach) # {{{
    ...
    if [[ -n $TMUX ]]; then
      ...
      cmd="tmux attach"
      $ENV_SCRIPTS/tmux/tm.sh --is-ssh && cmd="exec $cmd"
      [[ ! -z $var ]] && cmd+=" -t \"$var\""
    fi
    ;; # }}}
  path) # {{{
  ...
  exec) # {{{
    ...
    ;; # }}}
  monitors) # {{{
    if [[ -n $TMUX_SB_WORKER && -x $TMUX_SB_WORKER ]]; then # {{{
      pgrep -f "$(basename "$TMUX_SB_WORKER")" >/dev/null 2>&1 || $TMUX_SB_WORKER --tmux
    fi # }}}
    if [[ -n $REMINDER_MONITOR_CMD && -x $REMINDER_MONITOR_CMD ]]; then # {{{
      pgrep -f "$(basename "$REMINDER_MONITOR_CMD")" >/dev/null 2>&1 || $REMINDER_MONITOR_CMD monitor --tmux
    fi;; # }}} # }}}
  new) # {{{
    while [[ ! -z $1 ]]; do # {{{
      case $1 in
  ...
  pids) # {{{
    ...
    pane=$(
      ...
      | while read i; do
          ...
          pidInfo=
          if $IS_MAC; then
            lPid=$(pid-tree $pPid | sed -n 2p)
            [[ -n $lPid ]] && pidInfo=$(ps -p $lPid -o "command=")
          else
