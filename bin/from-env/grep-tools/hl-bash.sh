...
cmd_root="--color --binary-files=binary --text --line-buffered --color=yes"
params=() cnt=0 lines=false add_lines=false lines_colors=($(echo "48;5;235 48;5;232")) c= add_default=false
[[ -z $1 ]] && set -- ++
...
for (( i=0; i<$cnt; i++ )); do
  [[ ! -z $cmd ]] && cmd+=" | "
  cmd+="$ENV_SCRIPTS/grep-tools/grep-wrapper.sh --cmd-grep +tee=false +-fzf $cmd_root ${params[$i]} -e \"\$\""
done
[[ -z $cmd ]] && exit 1
if $add_lines; then
  [[ ! -z $cmd ]] && cmd+=" | "
