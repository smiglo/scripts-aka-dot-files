...
_ack-ag-wrapper() { # {{{
  ...
  if [[ -z $cmd ]] || ! which $cmd >/dev/null 2>&1; then
    echoe "Program [$cmd] not found, falling back to grep..."
    sleep 0.5
    while [[ ! -z $1 ]]; do
      case $1 in
      -|--) shift; break;;
    ...
    $ENV_SCRIPTS/grep-tools/grep-wrapper.sh --cmd-grep +tee=$use_tee +fzf=$use_fzf $fzf_params -R "$@" *
    return $?
  fi
  local params= prompt=
  case $cmd in
  ...
  if $use_tee || $use_fzf; then
    $ENV_SCRIPTS/grep-tools/output-to-file.sh --no-sort $use_tee +fzf=$use_fzf $fzf_params +fzf-p "--prompt '$prompt'" $cmd $params $@
  else
    $cmd $params $@
  fi
} # }}}
