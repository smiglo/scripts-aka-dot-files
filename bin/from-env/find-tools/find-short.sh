...
_find-short() { # @@ # {{{
  ...
  while [[ ! -z $1 ]]; do
    ...
    *) # {{{
      ...
      if [[ $# -gt 1 ]]; then
        ...
        progress --msg "$msg" --dots --cnt 30 --no-err || return 0
        fallback=true
      fi
      break;; # }}}
    esac
  ...
  if ! $fallback; then
    if ! $find_force && [[ -z $1 ]]; then
      echormf "fdfind $1"
      $ENV_SCRIPTS/grep-tools/output-to-file.sh $use_tee +fzf=$use_fzf +fzf-p "--prompt 'fd: $query> '" --no-err fdfind "$1"
    else
      local ddir="."
      [[ -d $1 ]] && ddir="$1" && shift
      params="-path '*/.git' -prune -o $params"
      echormf "find $ddir $params $([[ ! -z $1 ]] && echo "-name ${1@Q}") \"-print\""
      $ENV_SCRIPTS/grep-tools/output-to-file.sh $use_tee +fzf=$use_fzf +fzf-p "--prompt 'find: $query> '" --no-err \
        eval "find $ddir \
