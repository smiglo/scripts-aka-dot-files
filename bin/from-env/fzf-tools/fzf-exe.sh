...
_fzf-exe() { # @@ # {{{
  ...
  [[ ! -z $cmd ]]  || { echoe "Command not specified"; sleep 1; return 0; }
  [[ ! -z $file ]] || { echoe "File not specified"; sleep 1; return 0; }
  ...
  file="${file%%:*}"
  [[ ! -e $file ]] && file="${file%-*}"
  [[ -f $file || $cmd == 'prev' ]] || { echoe "Canot open [$file]"; sleep 1; return 0; }
  case $cmd in # {{{
  less | vim) # {{{
    case $cmd in
    less)       params="-N";;
  ...
  pane) # {{{
    local found=false pid= paneTmp=
    if [[ -z $pane ]]; then # {{{
      while read paneTmp pid; do
        if ! $IS_MAC; then
          pstree -Ac $pid | grep -q -e '---vim'
        else
          pstree $pid | grep -q -i -e 'vim'
        fi && found=true && break
    ...
    $found || { echoe "Vim not found in pane '$pane'"; sleep 1; return 0; }
    ;; # }}}
  *) # {{{
    { echoe "Unknown command [$cmd]"; sleep 1; return 0; };; # }}}
  esac # }}}
  case $cmd in # {{{
  prev) # {{{
    ...
    if [[ -f $file ]]; then
      ...
      if which highlight >/dev/null 2>&1; then
        ...
        cat $file
      fi | cat -n - | cut -c3- | tail -n+$first_line | head -n $prev_lines_cnt | $ENV_SCRIPTS/grep-tools/hl-bash.sh +cY "^\s*$line\s"
    elif [[ -d $file ]]; then
