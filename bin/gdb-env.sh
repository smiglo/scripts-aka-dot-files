...
gdbenv() { # @@ # {{{
  if [[ $1 == @@ ]]; then # {{{
    ...
    *)
      ...
      *) # {{{
        echo "--set-env"
        $IS_DOCKER && [[ $(uname -m) == 'x86_64' && -e $HOST/Library/com.apple.AppleMediaServices ]] && echo "--rosetta"
        ;; # }}}
      esac;;
    esac
    return 0
  ...
  --set-env) # {{{
    ...
    ;; # }}}
  --rosetta) # {{{
    if ! $IS_DOCKER || [[ $(uname -m) != 'x86_64' || ! -e $HOST/Library/com.apple.AppleMediaServices ]]; then
      eval $(die "only for x86-64 docker on mac")
    fi
    shift
    local f=$1; shift
    [[ -e $f ]] || eval $(die "File $f not exists")
    (
      ROSETTA_DEBUGSERVER_PORT=1234 $f & \
      gdb \
  ...
  *) # {{{
    ...
    [[ -e $f ]] || { echoe -w "Executable not found"; return 1; }
    local c="${2:-$(get-file-list -t -1 core.$f.\*)}"
    [[ -e $c ]] || { echoe -w "Core file for executable [$f] not found"; return 1; }
    echo -e "\nRunning: gdb $f $c\n"
    gdb $f $c;; # }}}
  esac
} # }}}
