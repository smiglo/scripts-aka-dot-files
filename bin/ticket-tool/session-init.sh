...
if [[ "$TMUX_SESSION" == "$sessionName" ]]; then # {{{
  ...
  # }}}
elif [[ -z $TMUX_SESSION ]] || ! tmux list-sessions -F '#S' | grep -q "^$sessionName\$"; then # {{{
  __init_session() { # {{{
    local sessionDir="$1" sessionName="$2"
    ! tmux list-sessions -F '#S' | grep -q "^$sessionName\$" || return 0
    ...
    set-title --from-tmux "${sessionName}:1.1" "Main"
    local TMUX=
