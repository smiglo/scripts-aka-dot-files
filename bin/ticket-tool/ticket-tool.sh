...
while read -r i; do
  i="${i//\@\@/\ }"
  ! echo -e "$values" | grep -q -F "$i" && TICKET_CONF_WAIT_ON_PHRASES_COMPL+=" '$i'"
...
getCompletionList() { # {{{
  ...
  if [[ $func != - ]]; then # {{{
    ...
    if [[ -z $1 ]]; then # {{{
      ret="${complMap["$cmd"]}"
      if [[ -z $ret ]]; then
        echo "$func" | grep -q "^@@ *\(|.*\)\?)" && hasOldCompl=true
      ...
      if [[ -z $ret ]]; then
        ...
        if echo "$func" | sed -n '/^'"$subCmd"')/,/^[^ ]/p'  | sed '$d' | grep -q "#.*@@[^:-]\|@@)\|== ['\"]\?@@['\"]\?"; then
          shift
        ...
        if $useDefaultCompletion; then
          ...
          local spaces="$(echo "$commandPart" | grep "^  \+case \$[0-9]" | sed 's/case.*$//')"
          if [[ ! -z "$spaces" ]]; then
...
@@)  # {{{
  ...
  if [[ -z $1 || $1 == 1 ]]; then # {{{
    ...
    cd) # {{{
      echo "-v --get"
      [[ -e $path_issue ]] && echo "$(cd $path_issue && find . -maxdepth 2 -type d | sed -e '/^\.$/d' -e 's|^\./||' -e '/^\./d')"
      ;; # }}}
    edit) # {{{
      echo "-v";; # }}}
    help) # {{{
...
edit) # {{{
  ...
  f="$issue_file"
  [[ $1 == "-v" ]] && echo $f && exit 0
  [[ -e "$(dirname "$f")/Session.vim" ]] && (cd "$(dirname "$f")"; vim-session;) && exit 0
  vim $f
  ;; # }}}
env) # {{{
  ...
  if ! $minimal; then
    for b in $(tmux list-buffers -F '#{buffer_name}' | grep -a "^${issue}-" | sort); do
