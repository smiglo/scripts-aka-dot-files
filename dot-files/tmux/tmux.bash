...
getIcon() { # {{{
  local char=
  [[ " $TMUX_ICONS " =~ \ $1:([^ ]*)\  ]] && echo "${BASH_REMATCH[1]}" || $BASH_PATH/env-ext.sh get get-unicode-char "$1"
} # }}}
defaults() { # {{{
...
status_git() { # @@ # {{{
  get-repo-name() { # {{{
    local repoName="$(git config utils.repo-name)"
    if [[ -z $repoName ]]; then
      case $gitdir in
      ...
      *.git)  repoName="${gitdir%.git}";;
      ...
      repoName=${repoName##*/}
      TMUX_STATUS_GIT_REPO_MAP+=' scripts:s'
      TMUX_STATUS_GIT_REPO_MAP+=' vim:v'
      TMUX_STATUS_GIT_REPO_MAP+=' \.runtime:rt'
      TMUX_STATUS_GIT_REPO_MAP+=' (.*)\.github\.io:${BASH_REMATCH[1]}'
      for i in $TMUX_STATUS_GIT_REPO_MAP; do
        [[ $repoName =~ ^${i%%:*}$ ]] || continue
  ...
  ${TMUX_STATUS_GIT_STATUS:-true} || return
  ...
  local justBranch="$(git config utils.fast-status)"
  if ! declare -Fx __git_ps1 >/dev/null || ! declare -Fx __git_eread; then # {{{
    ...
    if [[ -e $TMUX_STATUS_GIT_INTERNAL_PS1 ]]; then
      source $TMUX_STATUS_GIT_INTERNAL_PS1
  ...
  case $justBranch in # {{{
  true)
    printf "#[fg=colour10,bold] |#[fg=colour13,none] %s%s" "$(get-repo-name)" " $b?"
    return;;
  esac # }}}
  local statF=$MEM_KEEP/tmux-git
  local useCache=true cacheInterval="$(git config utils.cache-interval)"
  [[ -z $cacheInterval ]] && cacheInterval=${TMUX_STATUS_GIT_CACHE_INTERVAL:-2}
  if [[ $cacheInterval == "-" ]]; then # {{{
    useCache=${PS1_CFG_GIT_USE_CACHE:-false}
    cacheInterval=0
  fi # }}}
  declare -A git_stat=() stat_last_update=() stat_branch=() stat_out=()
  if $useCache && [[ -s $statF ]]; then # {{{
    source $statF
    if (( $EPOCHSECONDS - ${stat_last_update[$gitdir]:-0} < cacheInterval )) && [[ $b == ${stat_branch[$gitdir]} ]]; then
      echo "${stat_out[$gitdir]}"
      return
    fi
    stat_branch[$gitdir]="$b"
  fi # }}}
  stat="$(unset GIT_PS1_SHOWCOLORHINTS; __git_ps1 "%s")"
  if [[ $b == '-' ]]; then
    if [[ $stat =~ ^\((.*)\)(.*)$ ]]; then
      b=${BASH_REMATCH[1]}
      stat=${BASH_REMATCH[2]}
  ...
  stat="${stat/<>/$nonFF}"
  # brach mapping # {{{
  # TMUX_STATUS_GIT_BRANCH_MAP+=' devel/(.*):D/${BASH_REMATCH[1]}'
  # TMUX_STATUS_GIT_BRANCH_MAP+=' feat/(.*):F/${BASH_REMATCH[1]}'
  ...
  [[ -n $stat ]] && stat=" $stat"
  stat="$b$stat"
  local out="$(printf "#[fg=colour10,bold] |#[fg=colour13,none] %s%s" "$(get-repo-name)" " $stat")"
  if $useCache; then # {{{
    git_stat[$gitdir]="$stat"
    stat_last_update[$gitdir]=$EPOCHSECONDS
    stat_out[$gitdir]="$out"
    (
      declare -p git_stat
      declare -p stat_last_update
      declare -p stat_branch
      declare -p stat_out
    ) >$statF
  fi # }}}
  echo "$out"
...
status_right_extra() { # @@ # {{{
  ...
  for l in $TMUX_STATUS_RIGHT_EXTRA_SORTED; do # {{{
    if [[ -v "data[$l]" ]]; then # {{{
      ...
      logtime) # {{{
        ...
        ;; # }}}
      *) # {{{
        local i= ext="$BASH_PATH/env-ext.sh"
        for i in $($ext); do
          out="$($ext call=$i __util_tmux_extra $l)"
          [[ ! -z $out ]] && break
        done
        if ! ${dbg:-false}; then
          add_to_result "$out"
  ...
  printf "%b" "${ret# } "
  if false; then # {{{
    local title="$(tmux show -gv set-titles-string 2>/dev/null)"
    if [[ -n $title ]]; then
      title="${title//\\}"
      [[ "$title" == *" " ]] && title="${title% }" || title="$title "
      tmux set -g set-titles-string "$title"
    fi
  fi # }}}
  return 0
} # }}}
...
status_left_extra() { # {{{
  ...
  $ENV_SCRIPTS/tmux/tm.sh --is-ssh && overSSH=true
  if $checkSSH; then
    $overSSH && printf "%b" "$(getIcon ssh)"
    return 0
  fi
  TMUX_STATUS_LEFT_EXTRA_MAP= # Temporarily disabled
  [[ -n $TMUX_STATUS_LEFT_EXTRA_MAP ]] && info="$(echo -e "$TMUX_STATUS_LEFT_EXTRA_MAP" | sed -n '/^'$sName':/s/^'$sName'://p')"
...
set_title_extra() { # {{{
  ...
  if (( EPOCHSECONDS - ts >= 120 )) && ${TMUX_STATUS_LEFT_CHECK_SSH:-true}; then
    ts=$EPOCHSECONDS
    if $ENV_SCRIPTS/tmux/tm.sh --is-ssh; then
      isSsh=true
      [[ -n $sshIcon ]] || sshIcon=$(getIcon ssh)
    fi
  fi
  [[ -n $isSsh ]] || isSsh=false
...
set_title() { # {{{
  if ${TMUX_STATUS_LEFT_CHECK_SSH:-true} && ! $IS_VIRTUAL_OS; then
    tmux set -qg set-titles-string "#($HOME/.tmux.bash set_title_extra)"
  elif [[ -n $TMUX_TITLE ]]; then
    tmux set -qg set-titles-string "$TMUX_TITLE"
  else
    tmux set -qg set-titles-string "$(getIcon ${TMUX_ICON_HOST:-localhost})"
  fi
} # }}}
window_status_flags() { # {{{
  local flags="$1"
...
plugins() { # {{{
  ...
  tmux set -qg @fingers-ctrl-action "cat - | $SCRIPT_PATH/bin/oth/tmux-xclip --put"
...
pasteKey_worker() { # @@ # {{{
  ...
  local buff="$1" query="$2" v= keys="$(keep-pass --list-all)"
  if [[ ! -z $query ]]; then
...
new_window() { # {{{
  local currPaneId=$1 path="$2" doSplit=${3:-false} env_src= title=
  read env_src title < <(tmux display-message -p -t $currPaneId -F '#{pid}.#{s/^\$//:#{session_id}}.#{window_id} #W')
  local name="${4:-$title}"
  declare -a tmuxP=(
    -a
    -n "$name"
  ...
  if $doSplit; then
    tmux new-window "${tmuxP[@]}" \; \
      split-window -v -l 20% -d -c "$path"
  else
    tmux new-window "${tmuxP[@]}" -n "$name"
  fi
} # }}}
